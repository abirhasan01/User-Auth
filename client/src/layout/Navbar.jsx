...
const Navbar = () => {
  ...
  const sendVerificationOtp = async()=> {
    try {
      axios.defaults.withCredentials = true
      const { data } = await axios.post(backendUrl + "/api/auth/send-verity-otp");
      if(data.success){
          ...
          <div className="absolute hidden group-hover:block top-0 right-0 z-10 text-black rounded-full pt-10">
            <ul className="list-none m-0 p-2 bg-gray-100 text-sm">
              ...
              {!userData.isAccountVerified && (
                <li onClick={sendVerificationOtp} className="py-1 pc-2 hover:bg-gray-200 cursor-pointer">
                  Verify email
                </li>
              )}
              <li onClick={logout} className="py-1 pc-2 hover:bg-gray-200 cursor-pointer pr-10">
                Logout
              </li>
            </ul>
          </div>
